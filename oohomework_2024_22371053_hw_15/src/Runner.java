...
public class Runner {
    ...
    private void handleReturned(LibraryReqCmd request) {
        ...
        if (date.isAfter(book.getExpirationTime())) {
            ...
            // System.err.println("return! +1, cur = " + user.getCreditScore());
    ...
    private void handlePick(LibraryReqCmd request) {
        ...
        String userId = request.getStudentId();
        User user = users.get(userId);
        if (user.canPick(bookId)) { // 取书后该用户持有的书仍然满足借阅数量限制
            // 预约处存在一本为该用户保留的图书
    ...
    private void handleDonated(LibraryReqCmd request) {
        ...
        // System.err.println("donate +2, cur = " + user.getCreditScore());
        driftingCorner.addBook(new Book(bookId, user));
        PRINTER.accept(request);
    }
    ...
    private void handleRenewed(LibraryReqCmd request) {
        ...
        if (book.canRenew(date) && user.getCreditScore() >= 0) {
            appointmentsNum.putIfAbsent(bookId, 0);
    ...
    private void arrange4Open() {
        ...
        for (Book book : borrowedBooks) {
            if (book.expired(date) && !book.isHasDecreased()) {
                book.getCurrentUser().addCreditScore(-2);
                book.setHasDecreased(true);
            }
        }
        // PRINTER.move(date, moveInfos);
        ArrayList<LibraryMoveInfo> moveInfos = new ArrayList<>();
        ArrayList<LibraryMoveInfo> tmp = appointmentOffice.moveBooks2bookshelf(date, users);
