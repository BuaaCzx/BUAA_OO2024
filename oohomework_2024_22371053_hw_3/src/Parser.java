...
public class Parser {
    ...
    public Factor parseFactor() {
        ...
        } else if (lexer.peek().equals("dx")) { // 求导因子
            return parseDifferentialFactor();
        } else { // 常数因子：数字
    ...
    private Expression parseDifferentialFactor() {
        lexer.nextToken(); // 此时 curToken 指向 dx 后的表达式左括号（和exp里面是因子不同）
        Expression expr = parseExpressionFactor();
