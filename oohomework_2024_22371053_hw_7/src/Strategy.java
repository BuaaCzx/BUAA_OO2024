    ...
    */
    ...
    public Direction getDirection() {
        if (elevator.isDoubleCar()) {
            if (elevator.getCurrentCar() == 1
                    && elevator.getCurrentCarFloor(1) == elevator.getTransferFloor()) {
                return Direction.DOWN;
            } else if (elevator.getCurrentCar() == 2
                    && elevator.getCurrentCarFloor(2) == elevator.getTransferFloor()) {
                return Direction.UP;
            }
        }
        if (elevator.getMainRequestStatus().equals(RequestStatus.INSIDE)) {
            return elevator.getMainRequest().getDestination() > elevator.getEleCurrentFloor()
                    ? Direction.UP
                    : Direction.DOWN;
        } else {
            return elevator.getMainRequest().getBeginning() > elevator.getEleCurrentFloor()
                    ? Direction.UP
                    : Direction.DOWN;
        }
    }
    ...
    public void tryAddPassengers() {
        int currentFloor = elevator.getEleCurrentFloor();
        ...
        for (int i = 0; i < requests.size(); i++) {
            ...
            if (elevator.getPassengerNum() < maxPassengers
                    ...
                    && (!elevator.isDoubleCar()
                    || (elevator.isDoubleCar()
                    && elevator.getCurrentCar()
                    == request.getDoubleCarId(elevator.getTransferFloor())))) {
                elevator.doorOpen();
